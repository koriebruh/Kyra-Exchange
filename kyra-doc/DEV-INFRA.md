...
|-----------|----------|---------------|----------------|
| Custody (deposit addr, withdraw, balance) | `MockCustodyProvider` | **web3j self-custody** (web3j + OpenBao) | **YES** — Anvil + OpenBao, below |
...
key-never-whole security, an MPC vendor (Fireblocks/BitGo) can be added later as
another `CustodyProvider` bean — same interface, no logic change.
